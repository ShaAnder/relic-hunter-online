...
**Audio file casing mismatch, latent cross-platform bug.** `AudioController` was specified as `client/src/core/audio/AudioController.ts`; the actual file that exists is `audioController.ts`, lowercase. Works fine on Windows (case-insensitive filesystem) but would silently break the import on a case-sensitive filesystem — Linux CI, most deployment targets. Trivial fix (rename the file to match), just flagging it before it's forgotten.
